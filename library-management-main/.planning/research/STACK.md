# Stack Research — University Library Management System

**Project:** University Library Management System
**Researched:** 2026-06-10
**Overall Confidence:** HIGH (core stack is fixed; library choices are well-established 2024-2025 patterns)
...
SQLModel is built by the FastAPI author and is presented in the FastAPI docs as the recommended ORM. However, for this project, **raw SQLAlchemy 2.0 with async is the better choice** for these reasons:

1. **SQLModel's async story is incomplete.** SQLModel's async support is documented but lags behind SQLAlchemy's native async. For a CRUD-heavy app with multiple relationships (users, books, loans, requests), you will hit SQLModel's async limitations quickly.
2. **SQLAlchemy 2.0 is mature and async-stable.** The `AsyncSession` with `asyncpg` has been production-proven since 2022. Complex queries (e.g., join loans + books + users for overdue dashboard) are far cleaner with SQLAlchemy's select() API.
3. **Alembic integration is native.** Alembic was built for SQLAlchemy. It generates migrations automatically from your models — SQLModel's integration with Alembic requires extra manual steps.
4. **Pydantic schemas stay separate.** The FastAPI pattern of having distinct SQLAlchemy ORM models + Pydantic request/response schemas is more explicit and easier to maintain than SQLModel's dual-purpose models when schemas diverge (e.g., `BookCreate` vs `Book` vs `BookPublic`).

**Pattern to use:** SQLAlchemy `declarative_base()` models in `app/models/`, Pydantic schemas in `app/schemas/`, Alembic in `alembic/`.

### Async vs Sync in FastAPI Routes

Use **`async def`** for all route handlers because:
...
Use **`def`** only for pure CPU-bound utility functions. Never mix a sync blocking DB call inside an `async def` handler.

### JWT: PyJWT over python-jose

FastAPI's official documentation as of 2024-2025 **switched its recommendation from `python-jose` to `PyJWT`**. The reason: `python-jose` has unresolved CVEs and is less actively maintained. `PyJWT` is the canonical Python JWT library, maintained by the JWT Working Group. Use `HS256` algorithm for simplicity; RS256 if private/public key rotation is needed later (not needed for v1).

### Password Hashing: pwdlib[argon2] over passlib

FastAPI docs now recommend `pwdlib` with Argon2 over the previously recommended `passlib[bcrypt]`. Passlib is largely unmaintained. Argon2 is the winner of the Password Hashing Competition and is stronger than bcrypt. The API is simpler: `password_hash.hash(plain)` and `password_hash.verify(plain, hashed)`.

### Email: FastAPI BackgroundTasks + fastapi-mail over Celery

The PROJECT.md defines two email trigger points:
1. Borrow request approved/rejected (triggered by librarian action)
2. Loan overdue alert (triggered by a scheduled check)

For trigger #1 (request-time email): FastAPI's built-in `BackgroundTasks` is exactly the right tool. It runs the email send after the HTTP response is returned, requires no infrastructure, and is officially recommended for this use case.

For trigger #2 (scheduled overdue check): This needs a periodic task. The options are:
...
**Recommendation:** Use APScheduler in-process for v1. It runs a daily job to query overdue loans and send email alerts. Upgrade to ARQ/Celery only if needed. This avoids Redis as a required infrastructure dependency for the MVP.
...
Most of the React app's "state" is server state: lists of books, loan statuses, request queues. TanStack Query manages this with zero boilerplate: fetch, cache, invalidate on mutation. Redux is unnecessary — there is no complex shared client state. A small `useContext` or Zustand store for auth (JWT token + user role) is all the client state needed.
...
**shadcn/ui** is the correct choice because: (a) Tailwind is already in the stack, (b) you own the component code so there is no version lock-in, (c) it ships an excellent `DataTable`, `Dialog`, `Badge`, and form components that map directly to this app's needs (book catalog table, approval dialogs, overdue badges).
...
│   │   ├── auth.py
│   │   ├── email.py
│   │   └── scheduler.py        # APScheduler overdue jobs
│   └── dependencies.py         # get_current_user, require_librarian, etc.
├── alembic/
│   ├── env.py
│   └── versions/
├── tests/
...
**Note:** Include `mailhog` in dev Compose — it is a local SMTP sink that captures emails in a web UI without sending real emails. Invaluable for testing email triggers (borrow approval, overdue alerts).
...
The overdue-check scheduler (APScheduler) runs inside the FastAPI process. In Docker Compose dev this is fine. In production with multiple replicas, the job would run on every instance. **Mitigation for v1:** Run a single backend instance. Document this constraint. If multi-instance deployment is needed, switch to a database-backed scheduler or an ARQ job queue.

## Key Risk: JWT Storage on Frontend

Do not store JWT in `localStorage` — it is accessible to XSS scripts. Recommended pattern: store the access token in memory (React state / TanStack Query cache) and use a short expiry (15-30 min). Issue a `httpOnly` refresh token cookie for session persistence. For v1 a simpler approach is acceptable: short-lived JWT in memory only, re-login on page refresh. Document the tradeoff explicitly at implementation time.
